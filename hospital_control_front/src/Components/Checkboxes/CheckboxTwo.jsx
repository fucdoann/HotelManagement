...
const CheckboxTwo = (props) => {
  ...
  useEffect(() => {
    if (props.type == 'hotel') {
      const conveId = props.id
      const array = props.arrayConveHotel
      if (isChecked) {
        array.push(conveId)
      ...
      const conveId = props.id
      const indexConve = props.roomId
      const array = props.arrayConveHotel
      if (isChecked) {
        array[indexConve].push(conveId)
        props.setArrayConveHotel(array)
      }
      else {
        const index = array.indexOf(conveId)
  ...
  return (
    <div className='p-2'>
      <label
        htmlFor={`checkboxLabel${props.id}`}
        className="flex cursor-pointer select-none items-center"
      >
        <div className="relative">
          <input
            type="checkbox"
            id={`checkboxLabel${props.id}`}
            className="sr-only"
            onChange={() => {
